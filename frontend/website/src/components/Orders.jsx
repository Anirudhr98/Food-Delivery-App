...
export default function Orders() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const hasLiveOrders = orders.some(order => order.order_status !== "Delivered");
  ...
  const filteredOrders = useMemo(() => {
    return orderData.filter(order =>
      ...
      new Date(order.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }).toLowerCase().includes(searchQuery)
    );
  ...
  return (
    <>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 my-8">
        <div className="font-medium text-2xl mt-8 mx-8">Your Orders</div>
        {/* Search Bar */}
        <div className="mt-8 mx-8">
          <Input
            type="text"
            placeholder="Search Orders based on restaurant name or date .."
            value={searchQuery}
                  ...
                  <TableBody>
                    {filteredLiveOrders.map((order, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          {new Date(order.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })} at{" "}
                  ...
                  <TableBody>
                    {filteredPastOrders.map((order, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          {new Date(order.delivered_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })} at{" "}
